                        ...
                        <table id="demo-foo-addrow2" class="table table-bordered table-hover toggle-circle text-center" data-page-size="7">
                            <thead>
                                <tr>
                                    ...
                                    <th data-hide="phone, tablet">OPENED</th>
                                ...
                                <div class="d-flex">
                                    <div class="mr-auto">
                                        <div class="form-group">
                                            <a href="{{ URL::to('admin/proposals/create') }}">
                                                <div class="g-signin2" data-onsuccess="onSignIn" data-theme="dark"></div>
                                                <button class="btn btn-info btn-block btn-sm"><i class="icon wb-plus" aria-hidden="true"></i>Add New Proposal
                                                </button>
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <tbody>
                                @if($datas)
                                    @foreach($datas as $data)
                                        <tr>
                                            <td><a href="{{ URL::to('admin/proposals/'.$data->id.'/edit') }}">{{$data->name}}</a></td>
                                            <td>{{$data->user_name}}</td>
                                            <td>{{($data->status==1) ? 'Not Opened' : 'Opened'}}</td>
                                            <td>{{($data->status==3) ? 'Approved' : 'Not Approved'}}</td>
                                            <td>{{$data->expire_str}}</td>
                                            <td>{{$data->total_sum}}</td>
                                            <td>{{$data->price12_sum}}</td>
                                            <td>
                                                <a href="{{ URL::to('admin/proposals/delete').'/'.$data->id }}"><button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn"><i class="fa fa-bitbucket" style="font-size: 1.5em;" aria-hidden="true"></i></button></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
